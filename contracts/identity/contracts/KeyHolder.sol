...
contract KeyHolder is ERC725 {
    ...
    // General Nonce used for the next execution of the contract.
    ...
    mapping (bytes32 => Key)       keys;            // The keys that are stored in this contract.
    mapping (uint256 => bytes32[]) keysByPurpose;   // Show all keys for a given purpose.
    mapping (uint256 => Execution) executions;      // The executions of the contract.
        ...
        returns (bool)
    {
        ...
        if (_approve == true) {
            ...
            (bool success, bytes memory returnData) = executions[_id].to.call(abi.encode(executions[_id].data, 0));
            if (success) {
